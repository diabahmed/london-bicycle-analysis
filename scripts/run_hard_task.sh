...
# Hard Task Runner Script for London Bicycles Data Pipeline
# This script runs the hard task pipeline to calculate total distance covered
...
print_status "Running Hard Task Pipeline on Google Cloud Dataflow..."
...
if [ ! -f "src/pipeline/hard_task_pipeline.py" ]; then
    print_error "Hard task pipeline file not found: src/pipeline/hard_task_pipeline.py"
    exit 1
fi

if [ ! -f "src/transforms/hard_task.py" ]; then
    print_error "Distance transforms file not found: src/transforms/hard_task.py"
    exit 1
fi
...
# Run the pipeline
print_status "Starting Hard Task Pipeline execution..."
...
if [ $? -eq 0 ]; then
    print_success "Hard Task Pipeline completed successfully!"
    print_success "Results written to gs://$BUCKET_NAME/output/"
    print_status "Output format: start_station_id,end_station_id,ride_count,total_distance"
    ...
    print_error "Hard Task Pipeline failed with exit code $?"
    exit 1
fi
